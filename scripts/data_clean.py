...
logger_obj = Logger_Class("../logs/data_cleaning.log").get_logger()
           

class DataClean:
    def __init__(self,df,deep=False) -> None:
        self.logger = Logger_Class("../logs/data_cleaning.log").get_logger()
        
        if(deep):
            self.df = df.copy(deep=True)
        else:
            self.df = df
        #self.logger.info("DataClean method added!")
            
    def remove_unwnated_columns(self,cols):
        self.df.drop(cols,axis=1,inplace=True)
        self.logger.info("removing unwanted columns successful!")
        return self.df
    
    def remove_nulls(self) -> pd.DataFrame:
        self.logger.info("removing nulls successful!")
        return self.df.dropna()
       
    def remove_duplicates(self):
        rmv = self.df[self.df.duplicated()].index
        self.logger.info("removing duplicates successful!")
    ...
    def fix_outlier(self,cols):
        ...
        self.logger.info("removing outliers successful!")
    ...
    def dateParser(self,col,drop_date_col=True):
        ...
        self.logger.info("parsed data successfully!")
    ...
    def fix_outlier_with_median(self,Df,col):
        ...
        self.logger.info(f"outlier for {col} successfully fixed!")
        return Df
